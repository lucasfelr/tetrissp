...
static void lcd_read(uint8_t cmd, char *data ,uint32_t len)
{
    ...
    g_ioport_on_ioport.pinWrite(LCD_CMD,IOPORT_LEVEL_HIGH);
    g_ioport_on_ioport.pinCfg(IOPORT_PORT_01_PIN_02,((uint32_t)IOPORT_CFG_PORT_DIRECTION_OUTPUT | (uint32_t)((g_spi_lcdc.p_cfg->clk_polarity == SPI_CLK_POLARITY_HIGH) ? IOPORT_LEVEL_LOW : IOPORT_LEVEL_HIGH)));
    R_BSP_SoftwareDelay(5,BSP_DELAY_UNITS_MICROSECONDS);
    g_ioport_on_ioport.pinCfg(IOPORT_PORT_01_PIN_02,((uint32_t)IOPORT_CFG_PERIPHERAL_PIN | (uint32_t)IOPORT_PERIPHERAL_SCI0_2_4_6_8));
