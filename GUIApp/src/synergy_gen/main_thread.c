...
{ .channel = 2, .rate = I2C_RATE_STANDARD, .slave = 0x48, .addr_mode = I2C_ADDR_MODE_7BIT, .sda_delay = 300,
#define SYNERGY_NOT_DEFINED (1)            
...
static uint8_t queue_memory_g_input_queue[20];
...
void main_thread_create(void)
{
    ...
    UINT err_g_input_queue;
    err_g_input_queue = tx_queue_create (&g_input_queue, (CHAR *) "Queue", 1, &queue_memory_g_input_queue,
                                         sizeof(queue_memory_g_input_queue));
    if (TX_SUCCESS != err_g_input_queue)
    {
        tx_startup_err_callback (&g_input_queue, 0);
    }
